...
// newAddressTaproot returns a new ppcutil.AddressTaproot from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressTaproot(scriptHash []byte) ppcutil.Address {
	addr, err := ppcutil.NewAddressTaproot(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
		panic("invalid script hash in test source")
	}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			addrs: []ppcutil.Address{newAddressTaproot(
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2wsh, err := ppcutil.NewAddressWitnessScriptHash(hexToBytes("e981bd992a43650657"+
	...
	p2tr, err := ppcutil.NewAddressTaproot(hexToBytes("3a8e170b546c3b122ab9c175e"+
	...
	p2wpkh, err := ppcutil.NewAddressWitnessPubKeyHash(hexToBytes("748e50366adb8"+
	...
	tests := []struct {
		...
		{(*ppcutil.AddressWitnessPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ppcutil.AddressWitnessScriptHash)(nil), "", errUnsupportedAddress},
		{(*ppcutil.AddressTaproot)(nil), "", errUnsupportedAddress},
