...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a ppcutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "PAGzgNtTFFX2gJ3B33f2J6DXeA2uzHv1vG"
	address, err := ppcutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := ppcutil.Hash160(pubKey.SerializeCompressed())
	addr, err := ppcutil.NewAddressPubKeyHash(pubKeyHash,
	...
	lookupKey := func(a ppcutil.Address) (*btcec.PrivateKey, bool, error) {
...
func ExampleScriptTokenizer() {
	...
	// some other source.
	hash160 := ppcutil.Hash160([]byte("example"))
