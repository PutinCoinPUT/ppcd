...
func pubKeyHashToAddrs(hash []byte, params *chaincfg.Params) []ppcutil.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []ppcutil.Address
	addr, err := ppcutil.NewAddressPubKeyHash(hash, params)
	if err == nil {
...
func scriptHashToAddrs(hash []byte, params *chaincfg.Params) []ppcutil.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []ppcutil.Address
	addr, err := ppcutil.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []ppcutil.Address, int, error) {
	...
	if data := extractPubKey(pkScript); data != nil {
		var addrs []ppcutil.Address
		addr, err := ppcutil.NewAddressPubKey(data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if details.valid {
		...
		addrs := make([]ppcutil.Address, 0, len(details.pubKeys))
		for _, pubkey := range details.pubKeys {
			addr, err := ppcutil.NewAddressPubKey(pubkey, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if hash := extractWitnessPubKeyHash(pkScript); hash != nil {
		var addrs []ppcutil.Address
		addr, err := ppcutil.NewAddressWitnessPubKeyHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if hash := extractWitnessV0ScriptHash(pkScript); hash != nil {
		var addrs []ppcutil.Address
		addr, err := ppcutil.NewAddressWitnessScriptHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if rawKey := extractWitnessV1KeyBytes(pkScript); rawKey != nil {
		var addrs []ppcutil.Address
		addr, err := ppcutil.NewAddressTaproot(rawKey, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
