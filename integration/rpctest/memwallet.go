...
import (
	...
	"github.com/PutinCoinPUT/ppcd/rpcclient"
...
type memWallet struct {
	...
	addrs map[uint32]ppcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]ppcutil.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*ppcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          ppcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (ppcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []ppcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt ppcutil.Amount,
	feeRate ppcutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := ppcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate ppcutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate ppcutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt ppcutil.Amount
	for _, output := range outputs {
		outputAmt += ppcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() ppcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance ppcutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (ppcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := ppcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
