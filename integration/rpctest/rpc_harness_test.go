...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt ppcutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(ppcutil.Amount(5 * ppcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(ppcutil.Amount(500 * ppcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(ppcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*ppcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, ppcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := ppcutil.Amount(250 * ppcutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = ppcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := ppcutil.Amount(50 * ppcutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := ppcutil.Amount(numMatureOutputs * 50 * ppcutil.SatoshiPerBitcoin)
