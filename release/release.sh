...
PKG="github.com/PutinCoinPUT/ppcd"
...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" btcdemoppc/ppcd
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -o ppcutil -trimpath -ldflags="-s -w -buildid=" btcdemoppc/ppcd/cmd/btcctl
    cd ..
