...
func (s *fakeChain) FetchUtxoView(tx *ppcutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *ppcutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *ppcutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   ppcutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*ppcutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee ppcutil.Amount,
	signalsReplacement bool) (*ppcutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput ppcutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*ppcutil.Tx, error) {
	txChain := make([]*ppcutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, ppcutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := ppcutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *ppcutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee ppcutil.Amount,
	signalsReplacement, confirmed bool) *ppcutil.Tx {
...
func testPoolMembership(tc *testContext, tx *ppcutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   ppcutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *ppcutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*ppcutil.Tx, []*ppcutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*ppcutil.Tx, []*ppcutil.Tx) {
				...
				return tx, []*ppcutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*ppcutil.Tx, []*ppcutil.Tx) {
				...
				return tx, []*ppcutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = ppcutil.SatoshiPerBitcoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*ppcutil.Tx, []*ppcutil.Tx) {
				...
				return childReplace, []*ppcutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
