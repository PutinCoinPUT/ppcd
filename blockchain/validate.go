...
const (
	...
	baseSubsidy = 50 * ppcutil.SatoshiPerBitcoin
...
func IsCoinBase(tx *ppcutil.Tx) bool {
...
func IsFinalizedTransaction(tx *ppcutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(chainParams *chaincfg.Params, tx *ppcutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > ppcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *ppcutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *ppcutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *ppcutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *ppcutil.Block, chainParams *chaincfg.Params, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *ppcutil.Block, chainParams *chaincfg.Params, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *ppcutil.Tx) (int32, error) {
...
func CheckSerializedHeight(coinbaseTx *ppcutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *ppcutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *ppcutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *ppcutil.Tx, txHeight int32, nTimeTx int64, utxoView *UtxoViewpoint, moneySupply int64, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", ppcutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > ppcutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				ppcutil.Amount(originTxSatoshi),
				ppcutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > ppcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *ppcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *ppcutil.Block) error {
	b.chainLock.Lock()
