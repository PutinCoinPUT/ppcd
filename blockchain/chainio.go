...
func (b *BlockChain) FetchSpendJournal(targetBlock *ppcutil.Block) ([]SpentTxOut, error) {
	b.chainLock.RLock()
...
func dbFetchSpendJournalEntry(dbTx database.Tx, block *ppcutil.Block) ([]SpentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := ppcutil.NewBlockWithMetas(b.chainParams.GenesisBlock, b.chainParams.GenesisMeta)
...
func (b *BlockChain) initChainState() error {
	...
	err = b.db.View(func(dbTx database.Tx) error {
		...
		for ok := cursor.First(); ok; ok = cursor.Next() {
			...
			meta, err := ppcutil.MetaFromBytes(metaBuf)
			if err != nil {
				return err
			}
		...
		blockWeight := uint64(GetBlockWeight(ppcutil.NewBlock(&block)))
...
// raw block for the provided node, deserialize it, and return a ppcutil.Block
// with the height set.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*ppcutil.Block, error) {
...
func dbStoreBlockNode(dbTx database.Tx, node *blockNode) error {
	...
	/*
		sMeta, err := ppcutil.MetaToBytes(node.meta)
		if err != nil {
			return err
		}
...
func dbStoreBlock(dbTx database.Tx, block *ppcutil.Block) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(blockHeight int32) (*ppcutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*ppcutil.Block, error) {
