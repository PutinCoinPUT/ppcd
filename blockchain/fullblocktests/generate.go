...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: ppcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ppcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*ppcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ppcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount ppcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee ppcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee ppcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee ppcutil.Amount) *wire.MsgTx {
