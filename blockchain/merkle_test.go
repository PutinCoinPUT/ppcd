...
func TestMerkle(t *testing.T) {
	block := ppcutil.NewBlock(&Block100000)
...
func makeTxs(size int) []*ppcutil.Tx {
	var txs = make([]*ppcutil.Tx, size)
	for i := range txs {
		tx := ppcutil.NewTx(wire.NewMsgTx(2))
		tx.Hash()
		txs[i] = tx
	}
	return txs
...
func benchmarkRollingMerkle(b *testing.B, txs []*ppcutil.Tx) {
	b.ReportAllocs()
	b.ResetTimer()
...
func benchmarkMerkle(b *testing.B, txs []*ppcutil.Tx) {
	b.ResetTimer()
	b.ReportAllocs()
