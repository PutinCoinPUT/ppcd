...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *ppcutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *ppcutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *ppcutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a ppcutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := ppcutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a ppcutil.Address) bool {
	switch a := a.(type) {
	case *ppcutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a ppcutil.Address) {
	switch a := a.(type) {
	case *ppcutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected ppcutil.Block
type notificationBlockDisconnected ppcutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *ppcutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *ppcutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *ppcutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, ppcutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(btcjson.RpcVersion1, nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*ppcutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *ppcutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *ppcutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *ppcutil.Tx, block *ppcutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *ppcutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *ppcutil.Tx, block *ppcutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := ppcutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *ppcutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *ppcutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *ppcutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *btcjson.RescanCmd, lookups *rescanKeys, minBlock,
	...
	*ppcutil.Block, *chainhash.Hash, error) {
