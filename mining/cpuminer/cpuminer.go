...
type Config struct {
	...
	ProcessBlock func(*ppcutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *ppcutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), ppcutil.Amount(coinbaseTx.Value))
	return true
}
