...
const (
	...
	MinHighPriority = ppcutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr ppcutil.Address) (*ppcutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *ppcutil.Tx, height int32) error {
...
func logSkippedDeps(tx *ppcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//	|  <= policy.BlockMinSize)          |   |
//	 -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress ppcutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*ppcutil.Tx, 0, len(sourceTxns))
	...
	*/
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := ppcutil.NewTx(coinbaseTx.MsgTx().Copy())
...
func AddWitnessCommitment(coinbaseTx *ppcutil.Tx,
	blockTxns []*ppcutil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A ppcutil.Block should use saved in the state to avoid
